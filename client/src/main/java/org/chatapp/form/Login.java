...
import org.chatapp.event.EventMessage;
...
import org.chatapp.swing.PanelSlide;
import org.chatapp.event.EventLogin;
...
public class Login extends javax.swing.JPanel {
        ...
        PublicEvent.getInstance().addEventLogin(new EventLogin() {
            ...
            public void login(ModelLogin login, EventMessage message) {
                new Thread(() -> {
                    ...
                    Service.getInstance().getClient().emit("login", login.toJSONOBJECT(), (Ack) objects -> {
                        if (objects.length > 0) {
                            ModelMessage ms = new ModelMessage((boolean) objects[0], objects[1].toString());

                            if (ms.isAction()) {
                                Service.getInstance().setUser(new ModelUserAccount(objects[2]));
            ...
            public void register(ModelRegister data, EventMessage message) {
                Service.getInstance().getClient().emit("register", data.toJsonObject(), (Ack) os -> {
                    if (os.length > 0) {
    ...
    private void initComponents() {
        ...
        pic.setImage(new javax.swing.ImageIcon(getClass().getResource("/org/chatapp/icon/login_image.png"))); // NOI18N
